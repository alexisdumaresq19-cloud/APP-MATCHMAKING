...
| **P2-S3 — Apprentissage et exploitation** | Livré : bilan après l'événement (« Avez-vous conclu une affaire? »), résumé par événement, suggestions d'ajustement de la matrice d'après les bilans (D-38), purge automatique des profils inactifs avec préavis (D-39), Lighthouse en CI. Reporté à un jalon ultérieur : double authentification (TOTP) pour les propriétaires, Sentry optionnel. | `CRON_SECRET` sur Vercel; compte Sentry si souhaité |
